...
- 常に日本語で応答してください
- Typescriptのファイル編集後は bun lint, bun type-check, bun buildを実行してコード品質を確認してください。必要があればクリーンになるまで修正してください
- Typescriptではない場合はbun lint, bun type-check, bun buildは必要ありません。
...
- ビルドするときにはlocal/features/ に移動して bun build を実行してください。
...
- bun build:comment-filter2 などのコマンドで個別ビルドして時間を短縮できます。詳細はlocal/features/package.jsonを参照してください。
- mylist2, comment-filter2, mlink-video-controllerを更新したときは併せてvideo-playerもビルドしてください。一緒にバンドルされているためです。
