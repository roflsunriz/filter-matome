...
コマンド画面の「既存コマンドをすべて除去してから適用」で切り替える。

- オフ（デフォルト）: 設定したコマンドと同じカテゴリーだけを置換し、位置や特殊効果など別カテゴリーの既存コマンドは保持する。
- オン: コメントの既存コマンドをすべて除去してから、設定したコマンドを適用する。

例えば既存コマンドが `small,red,ue,184`、通常コメントの設定が `medium,blue` の場合、オフでは `ue,184,medium,blue`、オンでは `medium,blue` になる。
