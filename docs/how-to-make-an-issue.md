...
GitHubに不慣れな方向けに、バグ報告（Issue）や機能提案（Feature Request）を投稿する手順をまとめました。ブラウザさえあれば進められるので、初めてでも順番に進めてみてください。

## 1. 事前準備
- GitHubアカウントを作成・サインインする  
  1) https://github.com/signup を開く → 画面の案内に従って登録  
  2) 登録メールに届く確認リンクをクリック  
  3) https://github.com にサインイン（右上の「Sign in」）
- filter-matomeリポジトリを開く  
  https://github.com/roflsunriz/filter-matome を開いておく（ブックマーク推奨）
- 報告に必要そうな情報をメモしておく  
...
2) 最新版で再現するか確認  
   - 可能なら最新版に更新しても同じかを見る
3) 再現手順を整理  
   - いつ・どの画面で・どの操作をしたかをメモ（後でそのまま貼り付けられます）
...
- **代替案 / 参考資料 / 優先度 / 追加情報**: 分かる範囲で記入。迷ったら空欄でも構いません
...
分からない点があれば、遠慮なくIssue内で「ここが不明です」と書いてください。必要事項が揃っていなくても、分かる範囲で投稿すれば一緒に整理できます。
