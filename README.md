...
[![Latest Version](https://img.shields.io/badge/latest-%23254-blue)](https://github.com/roflsunriz/filter-matome/releases/latest)
...
- **nlFilter API状態表示**: CommonHeaderの独立した`filter-matome`メニューで、再生速度同期・全編先読み・コメント再取得・コメントメニュー挿入・通知表示更新をページ開始時と180秒ごとに自動検査し、有効／自動検査中／未検出／版不一致／検査失敗を確認
- **背景画像設定**: 視聴ページの背景をカスタマイズ
- **プレミアム勧誘非表示**: 煩わしい勧誘要素を完全除去
- **広告生成・通信遮断**: 公式資産の広告コンポーネントとローダーをブラウザーへ渡す前に無力化し、残った広告要求もNicoCache_nlの上流接続前に遮断
...
- **全編先読み**: 公式プレイヤーで再生位置を保ちながら動画全体を読み込み、進捗を表示。解除すると通常の先読みに戻る
- **A-Bリピート**: 開始点A・終了点Bを現在位置または時刻入力で指定して区間を繰り返す。一時停止を維持し、動画切り替えで解除
...
├── DestroyAds.java/.class                # 広告生成コードの書き換えと上流通信前の遮断
