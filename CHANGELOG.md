...
更新時は101番フィルターとfeaturesを同じ版に揃え、視聴ページを再読み込みしてください。Firefoxで旧挙動が残る場合は、開発ツールの設定でHTTPキャッシュを一時的に無効化して再読み込みし、設定を戻して通常の再読み込みでも反映を確認してください。

### Added

- 【mlink-video-controller】途中から視聴した動画も全体を先に取得できるよう、現在の画質・音質をNicoCache_nlへ蓄積する「全編先読み」を追加した。ブラウザーの再生用バッファーを拡大せず、小さなストリームとして取得し、同じ品質の完成キャッシュを確認して100%とする。再生状態を保ち、進捗、中止、取得失敗を表示する。動画・品質の切り替えでは転送を止め、CommonHeaderのAPI一覧にも副作用のない自動検査を追加した。
...
- 【ドキュメント】不具合・質問・脆弱性の報告先と共有時の注意を判断できるよう、サポート、セキュリティ、コミュニティ行動指針を整備した。開発用Bunの要求はpackage.jsonの正本を参照するようにした。

### Fixed

- 【mlink-video-controller】360p配信の実寸法が354pxなどの場合も全編先読みの完成キャッシュを正しく確認できるよう、画質を映像の高さではなく公式の画質IDで照合する。低画質や高音質の接尾辞も正しく扱う。
- 【mlink-video-controller】巻き戻した映像と公式の時刻表示がずれないよう、公式プレイヤーのシーク開始・確定を通して内部時計も更新する。A-Bと通常のシーク・再生/停止を同じ接続へ統一し、再生位置同期APIの自動診断を追加した。
- 【mlink-video-controller】再生タブをキーボードでも操作できるよう、パネル内部の操作を画面外クリックとして扱わないようにした。狭幅・低高さでもパネルと操作ボタンが画面内へ収まるように調整した。

### Changed

- 【リリース・文書】リリース番号とlatestバッジを255へ更新し、配布アーカイブ形式と公開手順の案内を現行ワークフローへ揃えた。
...
- 【docs・comment-filter2】大量のルールでもフィルター性能を保ちやすいよう、文字列リテラル、単純な正規表現、ユーザーID NG、動画IDスコープを優先する設定上のヒントを追加した。

### Changed

- 【features・README】リリース番号を`252`へ更新し、latestバッジを`#252`へ変更した。
...
- 【common・CommonHeader】独自の位置計算を重ねたことでfilter-matome API状態メニューが不安定だったため、マウント、account余白予約、再配置、DOM監視、ホバー・フォーカス操作を`05_nicocache_menu.js`と同じ構造へ置き換えた。メニュー内容だけをAPI状態表示に変更し、表示順は`NicoCache → filter-matome → アカウント`に保つ。
...
- 【features・README】リリース番号を`250.1`へ更新し、latestバッジを`#250.1`へ変更した。
...
- 【features・README】リリース番号を`248`へ更新し、latestバッジを`#248`へ変更した。
