...
- 【mlink-video-controller】途中から視聴した動画も全体を先に取得できるよう、現在の画質・音質をNicoCache_nlへ蓄積する「全編先読み」を追加した。再生状態を保ち、進捗、中止、取得失敗を表示する。CommonHeaderのAPI一覧にも副作用のない自動検査を追加した。
...
- 【mlink-video-controller】長い動画でもブラウザーの再生用バッファー容量に左右されず先読みできるよう、全編保持方式をNicoCache_nlへのストリーム取得に変更した。同じ画質・音質の完成キャッシュを確認してから100%とし、中止・動画や品質の切り替えで転送を止める。
- 【mlink-video-controller】A-Bで戻った映像と公式の時刻表示がずれないよう、公式mediaのシーク開始・確定を通して内部時計も更新する。通常のシーク・再生/停止も同じ接続へ統一し、再生位置同期APIの自動診断を追加した。
