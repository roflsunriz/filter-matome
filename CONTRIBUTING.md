...
正式な手順は[how-to-update.md](how-to-update.md)を参照してください。前回のリリースタグからの全差分を確認し、版番号・READMEバッジ・CHANGELOGを揃えて、脆弱性検査と全体検証を行います。mainのプッシュ後にCIの成功を確認してから、新しい`#<番号>`タグを公開します。

GitHub Actionsがfeaturesとmatome-toolboxをビルドし、`filter-matome-<番号>.7z`とCHANGELOGの該当節を含むリリースを公開します。公開後はワークフローの成功、タグのコミット、配布物の内容を確認してください。失敗時はログから原因を確認し、公開済みタグを削除・付け替える方法を通常の復旧手順にしないでください。
