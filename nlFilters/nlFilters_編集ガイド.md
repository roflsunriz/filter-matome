...
| `101_disable_official_function.txt` | serverContextの設定JSONを保持し、公式プレイヤーとmlinkの再生速度・再生位置同期、全編先読み計画を版付きAPIで接続 |
...
- 101番の`FilterMatomeBufferingApi` v2は公式HLS sessionの`attachMedia`と`MANIFEST_PARSED`の隣接境界へ接続し、現在品質の映像・音声・初期化情報・鍵・Rangeの取得計画を公開する。再生用バッファーを拡大せず、NicoCache_nlへストリーム取得する。追従時は`local/features/src/sandbox/full-buffer-bridge.md`と`bun run sandbox:analyze-full-buffer`で複数世代を比較し、CommonHeaderの`全編先読み`項目・自動プローブ・型・テストも同時に更新する。
- 再生位置は`FilterMatomePlaybackControlApi` v1で公式mediaの`seek()`→`setCurrentTime()`へ接続する。動画要素への直接書き込み、確定処理だけの呼び出し、シーク中に公式getterの保留位置を無視する方法へ戻さない。`playback-control-bridge.md`と`bun run sandbox:analyze-playback-control`で契約を確認する。
