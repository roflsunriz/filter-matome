...
**⚠️ 重要**: 編集対象は **100_*.txt - 199_*.html** のファイルのみじゃ！

- `01_globalFilter.txt` から `99_*.txt` までのファイルは外部配布物のため、**絶対に編集してはならん**のじゃ
- これらは更新時に上書きされる可能性があるのじゃ
- カスタマイズは必ず 100番台以降のファイルで行うのじゃ
...
nlFilter は NicoCache_nl で解釈される特殊なフィルター言語じゃ。
...
- **重要性**: 他の機能の前提条件
- **編集時の注意**: このファイルを無効にすると他の機能が動作しなくなるのじゃ

### 101_disable_official_function.txt
- **役割**: 公式プレイヤーの制限機能を回避
- **対象**: 非プレミアム会員の制限解除
- **リスク**: 公式プレイヤーが動作しなくなる可能性あり
- **注意**: 導入・除去時はハード再読み込み（Ctrl+F5）が必要じゃ
...
- **対象**: 視聴ページ限定
- **依存**: `common.es.js` が必要
- **特徴**: 公式NGワード機能より遥かに強力じゃ

### 104_video_player.txt
- **役割**: 有料動画のキャッシュ再生
- **機能**: 視聴期限切れ動画の再生、削除済み動画検知
- **依存**: `common.es.js` が必要
- **重要**: `isNeedPayment` ステート書き換えが必須じゃ
...
### 1. ファイル分割の原則
- 400行以上のファイルは必ず分割するのじゃ
- 長すぎると処理に失敗する可能性があるからのう
...
**覚えておくこと**: nlFilter は強力じゃが、間違った使い方をするとサイトが正常に動作しなくなる可能性があるのじゃ。必ずバックアップを取って、慎重に編集するのじゃぞ！ 