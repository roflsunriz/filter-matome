...
- `verify-current-api-status-menu.ts`: NicoCache_nl経由のCookieなし一時タブでCommonHeaderの独立メニュー、再生速度・コメント再取得・コメントメニューAPIの状態、NicoCache直後とアカウント直前の順序、`05_nicocache_menu.js`互換のaccount／service配置、ポップオーバーの画面内表示を確認する。
