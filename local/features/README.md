...
- `bun run docs:capture`: NicoCache_nl経由の実ページとビルド済みSPAをChromeの一時セッションで開き、`docs/resources/`と`cover-images/`の利用者向け画面画像を更新する。
...
`docs:capture`は既存のChromeプロフィール、Cookie、IndexedDBを読み込みません。mylist2とwatch-historyの匿名サンプルだけを破棄可能な一時BrowserContextへ作成し、動画取得スケジューラーは予約を保存せず確認画面まで撮影します。公式Watch上のcomment-filter2、mlink-video-controller、背景画像設定も匿名セッションで撮影します。既定のプロキシーは`http://127.0.0.1:8080`、動画IDは`sm9`です。必要な場合は`DOCS_CAPTURE_PROXY`、`DOCS_CAPTURE_VIDEO_ID`、`DOCS_CAPTURE_BROWSER_CHANNEL`で変更できます。
