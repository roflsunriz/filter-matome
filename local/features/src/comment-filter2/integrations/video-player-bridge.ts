...
// 注意！ニコニコ動画の公式動画プレイヤーとの橋渡しでは無い！
// ニコニコ動画公式動画プレイヤーとの橋渡しはdata-interceptor.ts(Proxy)で行っている
...
export class VideoPlayerBridge implements IVideoPlayerBridge {
  ...
  static getInstance(): VideoPlayerBridge {
    if (window.__CF2_BRIDGE__) {
      window.logger?.info(
        "[CommentFilter2] VideoPlayerBridge already exists, returning existing instance",
      );
  ...
  private resolveCurrentSmid(
    globalData: CommentFilter2GlobalData | null,
  ): string {
  ...
  private startVideoPlayerDetection(): void {
    ...
    if (typeof MutationObserver !== "undefined") {
      ...
      window.logger?.debug(
        "[CommentFilter2] Using optimized MutationObserver for video_player detection",
      );
    } else {
      // フォールバック: 古い環境用の単発チェック
      setTimeout(() => this.checkVideoPlayerStatus(), 1000);
      window.logger?.info(
        "[CommentFilter2] Using timeout fallback for video_player detection",
      );
    }
  }
  ...
  private checkVideoPlayerStatus(): void {
    ...
    if (videoElement && !this.isVideoPlayerDetected) {
      ...
      window.logger?.info(
        "[CommentFilter2] video_player detected and integrated",
      );
  ...
  private setupVideoPlayerIntegration(): void {
    try {
      ...
      window.logger?.error(
        "[CommentFilter2] Failed to setup video_player integration:",
        error,
      );
    }
  }
  ...
  private hookCommentRetrieval(): void {
    ...
    if (
      window.videoPlayer &&
      typeof window.videoPlayer.getComments === "function"
    ) {
      const originalGetComments = window.videoPlayer.getComments.bind(
        window.videoPlayer,
      );

      window.videoPlayer.getComments = (
  ...
  private stopMonitoring(): void {
    if (this.retryTimeoutId !== null) {
      ...
      this.retryTimeoutId = null;
      window.logger?.debug(
        "[CommentFilter2] Monitoring stopped - retry timer cleared",
      );
    }
  }
  ...
  private startDataMonitoring(): void {
    ...
    if (this.hasSuccessfullyNotified || this.retryTimeoutId !== null) {
      window.logger?.debug(
        "[CommentFilter2] Monitoring already active or completed, skipping start",
      );
      return;
    }
    ...
    const checkGlobalData = () => {
      ...
      if (globalData.filteredData) {
        ...
        if (
          ...
          this.lastDataHash = "";
          window.logger?.debug(
            "[CommentFilter2] New video detected, resetting notification state",
          );
        }

        // 未通知または新しい動画の場合のみ通知
        if (!this.hasSuccessfullyNotified) {
          const success = this.notifyVideoPlayerWithDiffCheck(
          ...
          if (success) {
            ...
            this.stopMonitoring();
            window.logger?.info(
              "[CommentFilter2] Data transfer successful, monitoring stopped",
            );
            return;
          }
        }
      ...
      if (!this.hasSuccessfullyNotified && this.isVideoPlayerDetected) {
        ...
        window.logger?.debug(
          `[CommentFilter2] Retrying in ${this.backoffDelay}ms`,
        );
      }
    };
   ...
   */
  private notifyVideoPlayerWithDiffCheck(
    ...
    try {
      ...
      const dataString = JSON.stringify({
        ...
        commentCount:
          filteredData.data?.threads?.reduce(
      ...
      if (currentHash === this.lastDataHash) {
        window.logger?.debug(
          "[CommentFilter2] Data unchanged, skipping notification",
        );
      ...
      if (!skipRateLimit && now - this.lastNotificationTime < 1000) {
        window.logger?.debug(
          "[CommentFilter2] Rate limited, deferring notification",
        );
        return false;
      }
      ...
      if (videoElement) {
        ...
        window.logger?.info(
          "[CommentFilter2] Successfully notified video_player with filtered data",
        );
        return true;
      }
      ...
      window.logger?.error(
        "[CommentFilter2] Failed to notify video_player:",
        error,
      );
      return false;
    }
   ...
   */
  private adaptDataForVideoPlayer(
    apiResponse: CF2CommentApiResponse,
  ): VideoPlayerResponse {
    try {
  ...
  private executeForceSyncInternal(): void {
    ...
    if (this.hasSuccessfullyNotified) {
      window.logger?.debug(
        "[CommentFilter2] Already successfully notified, skipping force sync",
      );
      return;
    }
    ...
    if (!this.isVideoPlayerDetected) {
      window.logger?.warn(
        "[CommentFilter2] video_player not detected, cannot sync",
      );
      return;
    }
    ...
    if (globalData?.filteredData) {
      const success = this.notifyVideoPlayerWithDiffCheck(
        globalData.filteredData,
        true,
      ); // forceSync時はレートリミットを無視
      if (success) {
        this.hasSuccessfullyNotified = true;
        this.lastNotifiedSmid = this.resolveCurrentSmid(globalData);
        this.stopMonitoring(); // 成功時は監視を完全停止
        window.logger?.info(
          "[CommentFilter2] Force sync completed successfully",
        );
      } else {
        // 失敗した場合は監視を再開
        this.startDataMonitoring();
        window.logger?.warn(
          "[CommentFilter2] Force sync failed, restarting monitoring",
        );
      }
    } else {
      window.logger?.warn(
        "[CommentFilter2] No filtered data available for force sync",
      );
    }
  }
  ...
  public destroy(): void {
    ...
    window.logger?.info(
      "[CommentFilter2] VideoPlayerBridge destroyed and resources cleaned up",
    );
  }
}
