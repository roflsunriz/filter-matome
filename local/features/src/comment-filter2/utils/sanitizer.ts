...
export function sanitizeRegexPattern(
  ...
  try {
    ...
    window.logger?.warn(
      "[CommentFilter2] Invalid regex pattern:",
      pattern,
      error,
    );
    return null;
...
function normalizeRegexFlags(inputFlags: string): string {
  ...
  for (const flag of inputFlags) {
    if (supportedFlags.has(flag)) {
      ...
      window.logger?.warn(
        `[CommentFilter2] Unsupported regex flag ignored: '${flag}'`,
      );
    }
  }
  ...
  const normalizedFlags = flagOrder
...
export function sanitizeCommentCommands(commands: string[]): string[] {
  ...
  for (const command of validCommands) {
    ...
    if (/^#[0-9A-Fa-f]{6}$/.test(command)) {
      ...
      // 各カテゴリをチェック
      for (const [categoryName, categoryCommands] of Object.entries(
        EXCLUSIVE_COMMAND_CATEGORIES,
      )) {
