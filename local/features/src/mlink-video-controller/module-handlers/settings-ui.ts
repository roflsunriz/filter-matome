...
export class SettingsUI {
  ...
  private renderWatchPageSubModules(container: HTMLElement): void {
    const watchPageModule = this.moduleManager.getLoadedModulesMap().get('watch_page') as WatchPageModule;
    ...
    if (watchPageModule) {
      ...
      const subList = subContainer.querySelector('.sub-modules-list') as HTMLElement;
  ...
  private createModuleElement(config: ModuleConfig): HTMLElement {
    ...
    const template = this.shadowRoot.getElementById('module-item-template') as HTMLTemplateElement;
    if (!template) {
    ...
    // 説明設定
    const description = element.querySelector('.module-description') as HTMLElement;
    ...
    const exclusiveGroup = element.querySelector('.module-exclusive-group') as HTMLElement;
   ...
   * サブモジュール要素を作成
   */
  private createSubModuleElement(subModule: { id: string; name: string; description: string; enabled: boolean; isActive: () => boolean }): HTMLElement {
  ...
  private updateSubModuleStatus(subModuleId: string): void {
    ...
    if (subModuleItem) {
      ...
      if (watchPageModule) {
        ...
        if (subModule) {
          const status = subModuleItem.querySelector('.sub-module-status') as HTMLElement;
  ...
  private setupBackgroundModalEventListeners(): void {
    ...
    const nameInput = modal.querySelector('#modal-image-name-input') as HTMLInputElement;
    ...
    importButton?.addEventListener('click', () => {
      const fileInput = modal.querySelector('#modal-import-file-input') as HTMLInputElement;
      fileInput.click();
    });

    // インポートファイル選択
    const importFileInput = modal.querySelector('#modal-import-file-input') as HTMLInputElement;
  ...
  private async addModalImageFromFile(): Promise<void> {
    ...
    const fileInput = modal.querySelector('#modal-image-file-input') as HTMLInputElement;
    const nameInput = modal.querySelector('#modal-file-name-input') as HTMLInputElement;
