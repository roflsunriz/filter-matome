...
export class HeatmapManager {
    ...
    return new Promise((resolve, reject) => {
      ...
      const checkVideoReady = () => {
        ...
        if (videoElement && 
            ...
            !videoElement.paused) { // 再生が開始されている
  ...
  private renderToCanvas(canvas: HTMLCanvasElement, context: CanvasRenderingContext2D): void {
        ...
        if (mutation.type === 'childList') {
          ...
          mutation.removedNodes.forEach((node) => {
            if (node.nodeType === Node.ELEMENT_NODE) {
              ...
              if (element.querySelector?.('.heatmap-overlay-container') || 
