...
- **高さ制御**: 動画詳細情報の展開高と、NG設定・タグ編集・動画プレイヤー設定・ギフト・マイリスト追加パネルの高さは `watch-harajuku-module.ts` の実測値を CSS 変数へ反映して同期する。コメントリストの下端はプレイヤー操作バーを含む動画プレイヤー全体の下端に合わせる。タグ検索おすすめ欄と通常おすすめ欄はサイドバー内でビューポート連動の高さを持つスクロール領域として表示する
