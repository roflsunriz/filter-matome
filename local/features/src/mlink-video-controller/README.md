...
パネルとFABは `common/visual-theme.ts` の共通ダークトークンを使用し、cache-data-manager と同じサーフェス、境界、主操作色、角丸、影へ揃えます。動画上での視認性や操作性に関わる配置・サイズはモジュール固有の責務です。
...
- **役割**: `startMlinkVideoController()`による明示的な初期化・統合
...
- **背景画像設定ヘルプ**: 背景画像設定モーダル右側に `docs/USAGE.md` の要点を表示し、`local` 配下URL・ファイル選択保存・外部URL注意を確認できる
...
- **テスト方針**: `tests/README.md` の実環境再現方針に従い、`tests/mlink-video-controller.spec.ts` で各ボタンのクリック、input/change、検索、コピー、シーク、リンク無効状態を実ブラウザDOMで検証する
...
- **機能**: 原宿風CSS注入、再生数・コメント数・マイリスト数・投稿日時の集約表示、共通ヘルパーがwatchページのmetaタグから取得する `apiData` を基に再構築した投稿者アイコン・投稿者名・投稿動画リンク・投稿者操作の右上表示、詳細情報内の重複する統計・投稿者ブロックの非表示、ランキング・ジャンル・シリーズの一行表示、ライト/ダークテーマ切替、カラースキーム/背景画像の優先切替、背景画像優先時のタグ一覧背景透明化、背景セレクターとの同時使用、SPA遷移対応、コモンヘッダーのアカウント・フォロー新着・お知らせホバーメニュー配色保護
...
- **スタイル境界**: 汎用のボタン・トリガー形状は公式プレイヤーの `#root` 配下だけへ適用し、スタンドアロンの video-player UI を上書きしない
...
- `nico-info-page-module.ts` - [ニコニ広告のお知らせページ](https://blog.nicovideo.jp/niconews/category/nicoad)でデイリー福引をハイライトする機能。モジュール一覧のリンクから対象ページへ移動可能
...
- **キャッシュ削除**: `FilterMatomeCacheControl` の `POST /cache/filter-matome/v1/remove` を動画ID単位で呼び出す。削除対象はHLSに固定し、完了済み・停止済みHLSは即時削除、ダウンロード中HLSは完了または中断後の削除予約として扱う。応答の `completed` / `pending` / `partial` / `failed` を通知へ反映し、MP4・FLV・SWFは削除しない
...
- `mylist2.ts` - Mylist2操作。SPA遷移後の現在URLから動画ID・検索語を解決し、動画情報取得中やマイリスト選択中に遷移した場合は遷移前の対象を誤登録しないよう中止する。視聴ページでは動画、検索・タグ・マイリスト検索ページではキーワードを追加する
- `playback.ts` - 再生制御
- `speed.ts` - 再生速度制御
- `volume.ts` - 音量制御
