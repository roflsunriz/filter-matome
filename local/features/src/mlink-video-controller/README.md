...
- **機能**: 原宿風CSS注入、再生数・コメント数・マイリスト数・投稿日時の集約表示、動画詳細から取得した投稿者アイコン・投稿者名・投稿動画リンク・投稿者操作の右上表示、ライト/ダークテーマ切替、カラースキーム/背景画像の優先切替、背景画像優先時のタグ一覧背景透明化、背景セレクターとの同時使用、SPA遷移対応、コモンヘッダーのアカウント・フォロー新着・お知らせホバーメニュー配色保護
