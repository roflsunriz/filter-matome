...
暗い単色背景、1px境界による階層、8〜10pxの角丸、主操作だけに使う青、抑制した影、明確なフォーカスリングを、他プロジェクトと共有するビジュアル基準とします。共通値は `common/visual-theme.ts` に同期し、変更時は他画面への影響も確認してください。
...
- **機能**: ヘッダー構築、動画カード生成、フォント読み込み、品質バッジ、利用不可バッジ、`FilterMatomeCacheControl` APIによるHLS限定の一括削除・削除予約、getthumbinfo による不足メタデータ取得
...
- **機能**:
  ...
  - 可視範囲 + バッファ分のみDOM生成
  - スペーサー要素による全体高さ維持
  - `styles/styles.ts` のカード最小幅・ギャップに合わせた動的カラム数計算
- **編集タイミング**: パフォーマンス改善、バッファサイズ調整

#### `coordinators/event-coordinator.ts` - イベント調整
- **役割**: ユーザー操作の処理・調整
- **機能**: クリックイベント、キーボード入力、API詳細取得、モーダル表示、`FilterMatomeCacheControl` APIによるHLS限定削除、スクロールトップボタン
...
- **機能**:
  ...
  - プレースホルダー表示
  - エラー時のフォールバック画像
  - 読み込み済みURLの再表示時にプレースホルダーへ戻さないURLキャッシュ
  - 仮想スクロールで描画済みのカードは即時プリロードしてスクロール時のちらつきを抑制
...
- **役割**: 動画カードのHTML構造定義
- **機能**: コンパクトなサムネイル（遅延読み込み対応）、タイトル、メタデータ、主操作の再生ボタン、保存・削除をまとめたその他メニュー
- **編集タイミング**: カードレイアウト変更、新しいボタン追加

#### `templates/header-template.ts` - ヘッダーテンプレート
- **役割**: ヘッダー部分のHTML構造定義
- **機能**: ナビゲーション、ラベル付き検索ボックス、Mylist2・watch-historyを含むサイトリンク
- **編集タイミング**: ヘッダー機能追加、ナビゲーション変更

#### `styles/styles.ts` - CSSスタイル (最大ファイル)
- **役割**: 全体のスタイル定義
- **機能**: 
  - 1行あたりの表示件数を増やすコンパクトなグリッドレイアウト
  - 仮想スクロールの行高計算と一致する300px固定高カード
  - 検索・フィルター・ソートを集約した操作バーと、危険な一括操作を分離したメニュー
  - 長時間の管理作業でも眩しさを抑えるダークテーマ
  - 仮想スクロールコンテナスタイル
  - 検索結果モーダルスタイル
  - フィルター・ソートUIスタイル
  - レスポンシブ対応、テーマ
...
- **仮想スクロール**: `renderers/virtual-scroll-renderer.ts` (カード最小幅・ギャップ・行高計算調整)
