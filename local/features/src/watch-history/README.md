...
- 履歴一覧、動画詳細、シリーズ内動画のサムネイルは動画リンクとして描画する。`/local/`配信はnlFilterの自動挿入対象外なので、HTMLからNicoCache_nl正本の`local/ncnl_cache_display.js`、`local/15_cached_link_color.js`、`local/nl_cacheIcon.css`を明示的に読み込む。共通表示スクリプトを装飾スクリプトより先に実行し、CMAF/Domandの画質・音質に基づくキャッシュ品質バッジへ接続する。
