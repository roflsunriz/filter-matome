...
- **注意**: `onupgradeneeded` 内のスキーマ更新はバージョン変更トランザクション内で同期的に完了させること。`await` や別トランザクションを挟むと Firefox/Chrome でストア作成後の保存が失敗する可能性があります。
...
- **機能**: コメント整形、コマンド適用、DOMレイヤ制御、表示/非表示切替
...
- **機能**: 隠し video/HLS による候補URLの並列実再生プローブ、再生準備確認、候補失敗時のフォールバック、コメント読み込み並列化
