...
export class EventHandlers {
  ...
  async handleVideoDelete(event: Event): Promise<void> {
    ...
    if (
      await this.modalService.showCustomConfirm(
        `「${videoTitle}」をマイリストから削除しますか？`,
      )
    ) {
      try {
  ...
  async handleKeywordEdit(event: Event): Promise<void> {
    ...
    try {
      const newKeyword = await this.modalService.showKeywordEditModal(
        keywordId,
        currentKeyword,
      );
  ...
  async moveVideo(videoItem: HTMLElement, videoTitle: string): Promise<void> {
    try {
      ...
      const errorMessage =
        error instanceof Error ? error.message : "動画の移動に失敗しました";
      await this.modalService.showCustomAlert(errorMessage);
    }
  }
  ...
  async copyVideo(videoItem: HTMLElement, videoTitle: string): Promise<void> {
    try {
      ...
      const errorMessage =
        error instanceof Error ? error.message : "動画のコピーに失敗しました";
      await this.modalService.showCustomAlert(errorMessage);
    }
  }
  ...
  public async getVideoData(videoItem: HTMLElement): Promise<DBVideo | null> {
    ...
    const titleElement =
      videoItem.querySelector(".video-title-link") ||
    ...
    const thumbnailElement = videoItem.querySelector(
      ".video-thumbnail",
    ) as HTMLImageElement;
    ...
    const commentCount = parseInt(
      commentCountElement?.textContent?.replace(/[^0-9]/g, "") || "0",
    );
    const mylistCount = parseInt(
      mylistCountElement?.textContent?.replace(/[^0-9]/g, "") || "0",
    );
    ...
    const authorName =
      authorElement?.textContent?.replace("投稿者: ", "") || "不明";
    const length = lengthElement
      ? this.fileHelperService.parseLength(lengthElement.textContent || "")
      : 0;
    ...
    if (rawTags) {
      try {
        ...
        if (Array.isArray(parsed)) {
          const onlyStrings = (parsed as unknown[]).filter(
