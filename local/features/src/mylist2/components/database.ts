...
export class Mylist2DB {
    ...
    try {
      ...
      const [mylists, videos, keywords] = await Promise.all([
        new Promise<MylistInfo[]>((resolve, reject) => {
          ...
          mylistsRequest.onerror = () =>
            reject(new Error(this.toMessage(mylistsRequest.error)));
        }),
        new Promise<DBVideo[]>((resolve, reject) => {
          videosRequest.onsuccess = () => resolve(videosRequest.result);
          videosRequest.onerror = () =>
            reject(new Error(this.toMessage(videosRequest.error)));
        }),
        new Promise<KeywordInfo[]>((resolve, reject) => {
          keywordsRequest.onsuccess = () => resolve(keywordsRequest.result);
          keywordsRequest.onerror = () =>
            reject(new Error(this.toMessage(keywordsRequest.error)));
        }),
      ]);
      ...
      // 孤立したキーワードチェック
      const orphanedKeywords = keywords.filter(
        (k) => !mylistIds.has(k.mylistId),
      );
      if (orphanedKeywords.length > 0) {
        health.issues.push(
          `Found ${orphanedKeywords.length} orphaned keywords`,
        );
        health.isHealthy = false;
      }
  ...
  async createBackup(): Promise<string> {
    ...
    try {
      ...
      for (const storeName of storeNames) {
        ...
        backup.data[storeName] = await new Promise<unknown[]>(
          (resolve, reject) => {
      ...
      for (const storeName of storeNames) {
        ...
        await new Promise<void>((resolve, reject) => {
          ...
          clearRequest.onerror = () =>
            reject(new Error(this.toMessage(clearRequest.error)));
        });
        ...
        for (const item of data) {
          await new Promise<void>((resolve, reject) => {
            ...
            putRequest.onerror = () =>
              reject(new Error(this.toMessage(putRequest.error)));
          });
        }
      }
        ...
        try {
          ...
          for (let i = 0; i < stepsToExecute.length; i++) {
            ...
            try {
              ...
              window.logger?.info(
                `Migration step ${step.version} completed: ${step.description}`,
              );
            } catch (error) {
              window.logger?.error(
                `Migration step ${step.version} failed:`,
                error,
              );
              throw error;
            }
          ...
          if (db.objectStoreNames.contains("metadata")) {
            ...
            await new Promise<void>((resolve, reject) => {
              ...
              getRequest.onerror = () =>
                reject(new Error(this.toMessage(getRequest.error)));
            });
          }
  ...
  createInitialStores(db: IDBDatabase): void {
    ...
    if (!db.objectStoreNames.contains("videos")) {
      ...
      // v7以降での新インデックス
      try {
        videoStore.createIndex("tags", "tags", {
          unique: false,
          multiEntry: true,
        });
      } catch (e) {
