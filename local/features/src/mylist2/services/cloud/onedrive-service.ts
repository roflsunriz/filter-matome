...
export class OneDriveService {
  ...
  constructor(tokenFromConfig?: string | null) {
    this.accessToken =
      tokenFromConfig || localStorage.getItem("mylist2_onedrive_token");
  }
  ...
  private ensureAccessToken(): Promise<string> {
    ...
    if (!input)
      throw new Error("OneDrive アクセストークンが設定されていません");
  ...
  private async ensureBackupFolder(): Promise<string> {
    ...
    if (!res.ok) {
      ...
      throw new Error(
        `OneDrive フォルダ一覧取得失敗: ${res.status} ${res.statusText} ${text}`,
      );
    }
    const json = (await res.json()) as {
      value?: Array<{ id: string; name: string; folder?: unknown }>;
    };
    const existing = (json.value || []).find(
      (e) => e.folder && e.name === this.backupFolderName,
    );
    ...
    if (!create.ok) {
      ...
      throw new Error(
        `OneDrive フォルダ作成失敗: ${create.status} ${create.statusText} ${text}`,
      );
    }
  ...
  async uploadBackupZip(
    ...
    try {
      ...
      const put = await fetch(
        `https://graph.microsoft.com/v1.0/me/drive/items/${encodeURIComponent(folderId)}:/${encodeURIComponent(zipName)}:/content`,
        {
          method: "PUT",
          headers: { Authorization: `Bearer ${token}` },
          body: zipBlob,
      ...
      if (!put.ok) {
        ...
        throw new Error(
          `OneDrive アップロード失敗: ${put.status} ${put.statusText} ${text}`,
        );
      }
  ...
  > {
    ...
    const res = await fetch(
      `https://graph.microsoft.com/v1.0/me/drive/items/${encodeURIComponent(folderId)}/children?$select=id,name,size,lastModifiedDateTime`,
      {
        method: "GET",
        headers: { Authorization: `Bearer ${token}` },
      },
    ...
    if (!res.ok) {
      const text = await res.text().catch(() => "");
      throw new Error(
        `OneDrive 一覧取得失敗: ${res.status} ${res.statusText} ${text}`,
      );
    }
    const json = (await res.json()) as {
      value?: Array<{
    ...
    const files = (json.value || []).filter(
      (f) => /\.zip$/.test(f.name) && /^Mylist2_/.test(f.name),
    );
    return files
      .map((f) => ({
        id: f.id,
  ...
  async downloadBackupJson(fileId: string): Promise<string> {
    ...
    if (!resp.ok) {
      ...
      throw new Error(
        `OneDrive ダウンロード失敗: ${resp.status} ${resp.statusText} ${text}`,
      );
    }
