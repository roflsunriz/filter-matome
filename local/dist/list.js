...
class h {
  ...
  trigger(e, t) {
    (this.listeners.get(e) || []).forEach((a) => a(t));
  }
}
class p {
  constructor(e) {
  ...
  generateThumbnailUrl(e) {
    ...
    return `https://nicovideo.cdn.nimg.jp/thumbnails/${t?.[1]}/${t?.[1]}`;
  }
  // 品質情報をIDから直接解析
  parseQuality(e) {
    const t = e.match(/(\d+)p/);
  ...
  sortEntries(e) {
    return e.filter((t) => t && t.id).sort((t, r) => {
      const a = { nm: 1, sm: 2, so: 3 }, n = (d) => d.slice(0, 2), s = (d) => parseInt(d.match(/\d+/)?.[0] || "0", 10), o = n(t.id), l = n(r.id);
      return a[o] !== a[l] ? a[o] - a[l] : s(t.id) - s(r.id);
    });
  }
...
class g {
  ...
  async processBatch(e) {
    for (this.clearContainer(), this.renderQueue = [...e]; this.renderQueue.length > 0; ) {
      ...
      t.forEach((a) => {
        const n = this.createVideoCard(a);
        r.appendChild(n);
      }), this.container.appendChild(r), await new Promise((a) => requestAnimationFrame(() => a()));
    }
  }
  // 検索結果用最適化レンダリング
...
class m {
  ...
  async search(e) {
    ...
    return [...new Set(r.flatMap((a) => a.result))].filter((a) => typeof a == "string");
  }
  async rebuildIndex() {
...
class b {
  ...
  constructor(e, t, r) {
    this.loadFonts(), this.dataLoader = e, this.eventManager = t, this.progressManager = r, this.initializeTemplates(), this.createHeaderAndContainer(), this.renderer = new g(this.createVideoCard.bind(this), e), this.searchEngine = new m(e), this.setupSearchListener();
  }
  // フォント読み込みメソッド追加
  loadFonts() {
  ...
  initializeTemplates() {
    const e = document.createElement("div");
    e.className = "video-card", e.innerHTML = f(), this.templates.set("videoCard", e);
  }
  buildHeader() {
    const e = document.querySelector("header");
    e && e.remove();
    const t = document.createElement("header");
    return t.innerHTML = u(), t;
  }
  createVideoCard(e) {
...
class x {
  baseUrl = "https://ext.nicovideo.jp/api/getthumbinfo/";
  ...
  async fetchVideoInfo(e) {
    ...
    const r = await t.text(), a = this.parseResponse(r);
    return this.cache.set(e, a), setTimeout(() => this.cache.delete(e), 1800 * 1e3), a;
  }
  parseResponse(e) {
    const r = new DOMParser().parseFromString(e, "text/xml"), a = r.querySelector("error code")?.textContent;
    if (a)
      return {
        status: "error",
        errorCode: a,
        description: r.querySelector("error description")?.textContent || "不明なエラー"
      };
    const n = r.querySelector("thumb");
    return {
      status: "ok",
      title: n?.querySelector("title")?.textContent || "タイトル不明",
      description: n?.querySelector("description")?.textContent || "説明文がありません",
      duration: n?.querySelector("length")?.textContent || "0:00",
      views: parseInt(n?.querySelector("view_counter")?.textContent || "0") || 0,
      commentCount: parseInt(n?.querySelector("comment_num")?.textContent || "0") || 0,
      mylistCount: parseInt(n?.querySelector("mylist_counter")?.textContent || "0") || 0,
      author: n?.querySelector("user_nickname")?.textContent || r.querySelector("ch_name")?.textContent || "投稿者不明",
      uploadDate: n?.querySelector("first_retrieve")?.textContent || "不明",
      thumbnailUrl: n?.querySelector("thumbnail_url")?.textContent || "",
      tags: Array.from(n?.querySelectorAll("tags tag") || []).map((s) => s.textContent?.trim() || ""),
      fileSize: n?.querySelector("size_high")?.textContent || "0"
    };
  }
}
class y {
  client = null;
  async fetchVideoInfo(e) {
    return this.client || (this.client = new x()), this.client.fetchVideoInfo(e);
  }
}
class v {
  constructor(e, t, r) {
  ...
  setupHeaderEvents() {
    const e = document.querySelector("header");
    e && (e.querySelector("#searchBtn")?.addEventListener("click", () => {
      const t = e.querySelector("#searchInput").value;
      this.handleSearch(t);
    }), e.querySelector("#clearSearch")?.addEventListener("click", () => {
      e.querySelector("#searchInput").value = "", this.eventManager.trigger("searchClear");
    }), e.querySelector("#searchInput")?.addEventListener("keypress", (t) => {
      t.key === "Enter" && this.handleSearch(t.target.value);
    }));
  }
  setupCardEvents() {
    document.addEventListener("click", async (e) => {
      ...
      if (!t) return;
      const r = t.dataset.id, a = t.querySelector(".video-title").textContent || "", n = e.target.closest("button");
      if (!n) {
        await this.showDetailInfo(r);
        return;
      }
      n.classList.contains("play-btn") ? this.handlePlay(r) : n.classList.contains("save-video-btn") ? this.handleSaveVideo(r) : n.classList.contains("save-audio-btn") ? this.handleSaveAudio(r) : n.classList.contains("delete-btn") && this.handleDelete(r, a);
    });
  }
  async showDetailInfo(e) {
    const r = await new y().fetchVideoInfo(e);
...
タイトル : ${t}`) && window.open(`./rm?${e}`, "_blank");
  ...
  displayDetailModal(e) {
    const t = (s) => {
      ...
      return new Date(s).toLocaleDateString("ja-JP", o).replace(/\//g, "-");
    }, r = document.querySelector(".detail-modal");
    r && r.remove();
    const a = document.createElement("div");
    a.className = "detail-modal";
    const n = e.status === "error";
    a.innerHTML = `
      <div class="modal-content">
        <span class="close-btn">&times;</span>
        <h2>${n ? "エラーが発生しました" : e.title}</h2>
        <div class="modal-body">
          ${n ? "" : `<img src="${e.thumbnailUrl}" class="modal-thumbnail">`}
          <div class="modal-info">
            ...
            ` : `
              <p>投稿者: ${e.author}</p>
              <p>再生時間: ${e.duration}</p>
              <p>再生数: ${e.views?.toLocaleString()} 回</p>
              <p>コメント数: ${e.commentCount?.toLocaleString()}</p>
              <p>マイリスト数: ${e.mylistCount?.toLocaleString()}</p>
              <p>投稿日: ${t(e.uploadDate)}</p>
              <div class="modal-tags">${e.tags?.map((s) => `<span>${s}</span>`).join("") || ""}</div>
            `}
          </div>
        </div>
      </div>
    `, a.addEventListener("click", (s) => {
      s.target === a && a.remove();
    }), a.querySelector(".close-btn")?.addEventListener("click", () => a.remove()), document.body.appendChild(a);
  }
}
const k = `
body {
  ...
  `;
async function w() {
  const i = new c(), e = new h(), t = new p(i), r = new b(t, e, i);
  new v(r, e, i), await r.renderAllEntries();
}
window.addEventListener("load", () => {
  const i = document.createElement("style");
  i.textContent = k, document.head.appendChild(i), w();
});
